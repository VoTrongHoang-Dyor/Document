...
- 📱💻 Khi Notification đến từ NSE nhưng Main App chưa kịp đồng bộ khóa mới (OOB Push Ratchet), UI **không** hiển thị ngay nội dung thực mà dùng một hàng Skeleton cố định:
  - Title dạng generic: `"Thông báo bảo mật đang được giải mã..."` hoặc `"Đang giải mã an toàn..."`.
  - Subtitle hiển thị Blur/Skeleton bar (khối mờ 60–70% width), không lộ từ khóa hay nội dung nhạy cảm.
- 📱 Ngay khi Main App mở và đồng bộ xong `Push_Key`/MLS Epoch, hàng Skeleton được thay thế bằng nội dung thật bằng hiệu ứng fade 0.15s; nếu đồng bộ thất bại, Skeleton chuyển thành trạng thái lỗi an toàn `"Không thể giải mã — chạm để thử lại"` thay vì hiển thị rác.
