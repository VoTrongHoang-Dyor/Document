...
- 📱 **Hardcoded Skeleton UI + Progress Bar (Online Mode):** Trong toàn bộ thời gian Re-sync, UI render ngay **Skeleton UI** tĩnh (hardcoded placeholder, không cần fetch dữ liệu) kết hợp Progress Bar tuyến tính bên dưới header: *"🔄 Đang đồng bộ dữ liệu... X%"*. Skeleton dùng màu `#1E293B` (Slate-800), không có shimmer animation để tránh tốn GPU.
- 📱 **Event-driven SVG Radar Pulse (Mesh Mode):** Nếu Re-sync xảy ra trong Mesh Mode (không có Internet), biểu tượng SVG Radar Pulse được kích hoạt theo model Event-driven: opacity thay đổi (`0.3 → 1.0`) mỗi khi Lõi Rust dispatch tín hiệu `MESH_CHUNK_RECEIVED` — không dùng CSS animation loop, không tốn GPU frame liên tục.
- 📱 **Sub-300ms Cryptographic State Label:** Phía dưới Progress Bar, một label nhỏ cập nhật liên tục dưới ngưỡng 300ms (debounced): *"⚙️ Verifying Cryptographic State... [Merkle Chunk N/M]"*. Label chỉ cập nhật text content (`innerText`), không re-render DOM — tránh layout thrashing. Biến mất ngay khi commit transaction cuối cùng vào `hot_dag.db` thành công.
- 📱 **Skeleton Mode iOS — Background Thread Overflow Trigger:** Bắt buộc kích hoạt Skeleton Mode (không phải optional) khi Lõi Rust phát tín hiệu `IPC_EVENT: BACKGROUND_THREAD_PAYLOAD_OVERFLOW` — tức là khi payload TreeKEM/Epoch vượt quá ngưỡng xử lý an toàn của Background Thread trên Mobile (giới hạn cứng: 4KB cho NSE, 48MB cho BGTask). UI không được block. Toàn bộ logic giải mã nặng bị delegate sang Main Thread queue ngay khi user mở app. Skeleton hiển thị trạng thái: *"🔐 Đang đồng bộ mã hóa ở nền... Tin nhắn sẽ hiện sau vài giây."*
