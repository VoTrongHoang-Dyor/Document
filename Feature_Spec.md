...
- 📱💻🖥️ **Shadow Paging (Cách ly luồng ghi vào DB tạm):** Lõi Rust không ghi trực tiếp vào `cold_state.db`. Thay vào đó, mỗi phiên Hydration khai báo một file tạm `cold_state.db.shadow` — toàn bộ chunk được ghi vào Shadow file. File gốc `cold_state.db` không được đụng đến cho đến khi Shadow hoàn tất. Nếu bị kill giữa chừng, `cold_state.db` nguyên vẹn, Shadow file bị bỏ lại có thể detect và cleanup lần khởi động tiếp.
- 📱 **Idempotent Checkpointing (`Last_Successful_Chunk_Index`):** Sau mỗi chunk flush thành công vào Shadow file, Lõi Rust cập nhật một key-value `hydration_checkpoint = (snapshot_uuid, last_chunk_index)` vào `hot_dag.db`. Nếu bị kill, lần khởi tiếp theo đọc checkpoint → resume từ `last_chunk_index + 1`, bỏ qua chunk đã ghi. Đảm bảo tính **Idempotent** — không ghi đôi dữ liệu.
- 📱💻🖥️ **POSIX Atomic Rename (Tráo con trỏ file ở tầng OS):** Chỉ khi toàn bộ Shadow file đã ghi xong và fsync thành công, Lõi Rust gọi `rename(cold_state.db.shadow, cold_state.db)` — một syscall nguyên tử tại kernel. Filesystem đảm bảo hoặc rename xảy ra hoàn toàn, hoặc không xảy ra — không có trạng thái trung gian. Sau rename, `cold_state.db` chứa state hoàn chỉnh.
- 📱 **ZeroizeOnDrop (RAII) cho AES-256-GCM Key:** Mỗi chunk AES-256-GCM key được wrap trong Rust struct `ChunkKey(ZeroizeOnDrop)`. Ngay sau khi giải mã và flush chunk, biến `ChunkKey` ra khỏi scope → Rust tự động gọi `Drop` → `zeroize()` ghi đè `0x00` lên vùng RAM chứa key. Không có window nào để key tồn lại trong heap khi bị kill.
