...
* **Quản lý Khóa và Định danh (KMS & Identity):**
  ...
  * **Quy trình Xử lý Khủng hoảng Thiết bị (Device Loss Scenarios):**
    * 💻 **TH1 (Mất CẢ Laptop & Điện thoại) — Multi-Sig Executive Break-glass (ISO 27001 A.8.2):**

      > **Đã loại bỏ:** Cơ chế "1 YubiKey mở mọi thứ" (God Key / Single Point of Failure). Thay thế bằng Multi-Sig Shamir's Secret Sharing Quorum.
...
* Thiết lập `App Whitelist` cho phép nhúng các ứng dụng bên ngoài (như Zalo, Telegram) vào Sandbox, đồng thời bật các tính năng chống thất thoát dữ liệu (DLP) như chặn copy/paste, chặn kéo thả file.
* Ràng buộc ứng dụng `.tapp` khởi chạy hoàn toàn trong môi trường cách ly WASM Sandbox tuyệt đối nhằm ngăn chặn mọi hành vi đọc trộm dữ liệu hội thoại từ cửa sổ ứng dụng.
...
* **Quản lý Tin nhắn và Tài liệu (Collaboration & Conflict Resolution):**
  * 💻📱 **Quy trình Đề xuất Chỉnh sửa (Proposal Flow):** Người dùng bôi đen đoạn văn bản E2EE của người khác và chọn "Đề xuất sửa". Người gửi gốc sẽ nhận được thông báo yêu cầu duyệt.
  * 🗄️ **Phân quyền Granular Role-Based Access Control (RBAC):**
    * **Viewer:** Chỉ được xem, cấm bôi đen đề xuất.
  ...
  * 💻📱 **Quy trình Quản lý Nhánh (Author Workflow):** Chỉ người tạo tài liệu/tin nhắn (Author) hoặc Admin trong Group mới nhìn thấy Checkbox quản lý hàng loạt trong Bulk Resolution Pop-up.
  * 💻📱 **Luồng hoạt động Offline (Offline Flow Constraint):** Người dùng chỉ có thể thực hiện chỉnh sửa cá nhân (Local Edit) hoặc gửi tin nhắn mới khi offline. Mọi thao tác tạo nhánh (Branching) trên dữ liệu người khác bị vô hiệu hóa cho đến khi có mạng trở lại.
...
* Nhắn tin với đối tác qua các ứng dụng ngoài (Zalo, Telegram) ngay trong một cửa sổ duy nhất (Sandbox) mà không sợ bị dính mã độc chéo.
...
* **Cập nhật Không cần Mạng (Air-Gapped Patching):** Cho phép triển khai các bản vá lỗi siêu nhỏ (Delta Patch - BSDiff) cục bộ cho các máy trạm nằm trong mạng nội bộ đóng kín hoàn toàn với Internet.
* **Trạm Giám sát (Observability Pipeline):** Tích hợp sẵn chuẩn Prometheus (đo lường), Loki (lưu log JSON ẩn danh) và Tempo (theo dõi vết gói tin) để đảm bảo SLA hoạt động đạt mức 99.99%.
