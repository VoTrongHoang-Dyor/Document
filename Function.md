...
## Mục Mới: WASM & Network Function Specifications

### FUNC-01: terachat_proxy_request (Guest → Host → External)

```
Đầu vào: payload (Protobuf) từ .tapp trong WASM Sandbox
Luồng:
  1. Guest serialize payload Protobuf → ghi vào Shared Memory vùng riêng
  2. Host (Rust) polling/Async Interrupt đọc từ Shared Memory
  3. Validate:
     a. DID Signature của .tapp (Trusted Chain check)
     b. URL đích có trong Whitelist đã khai báo (Manifest) không?
     c. AI SLM Payload Inspection (ZKP nếu E2EE, Regex nếu plaintext)
  4. Rate Limit check (Token Bucket 50 req/s)
  5. Nếu Pass → tokio/reqwest thực thi Native I/O
  6. Nhận Response → Sanitize headers/body → serialize Protobuf
  7. Đẩy response lại Shared Memory cho Guest
Lỗi:
  - NETWORK_MESH_RESTRICTED (Survival Mesh mode)
  - PAYLOAD_TOO_LARGE (>5MB)
  - ERR_DID_UNAUTHORIZED (DID không hợp lệ)
  - ERR_RATE_LIMIT (Token Bucket cạn)
  - ERR_DOMAIN_NOT_WHITELISTED (URL ngoài Manifest)
```

### FUNC-02: Message TTL Lifecycle (Bitchat Store-and-Forward)

```
Quy tắc lưu trữ tạm tin nhắn mã hóa:
  - Desktop/Laptop (Relay/Heavy Node):
    TTL: 48-72 giờ
    Quota: 500MB - 1GB
    Behavior: Luôn nhận và giữ hộ gói tin Gossip
  - Mobile (Light Node):
    TTL: 24 giờ
    Quota: 50MB
    Behavior: CHỈ nhận tin nhắn của chính mình, không làm Relay
              (Ngoại lệ: Làm Temporary Super Node nếu không có Desktop)

Garbage Collection:
  - Scan định kỳ Database
  - TTL expired → Drop packet, giải phóng ổ cứng
  - Tombstone processed → Crypto-shred keys → Vacuum Database

False Positive DID (Bloom Filter):
  - Suspend .tapp thay vì Kill ngay
  - Gửi Merkle Proof verification đến nodes lân cận
  - Chỉ Kill sau khi 100% xác nhận
```

### FUNC-03: Sync Logic khi Device Reconnect

```
Khi thiết bị B online trở lại:
  1. State 1 (VPS) reconnect: Rust Core ping VPS → nhận delta messages từ Cloud
  2. Ưu tiên nguồn gần nhất:
     a. LAN Sync trước (mDNS discovery → kéo từ Desktop Relay gần nhất)
     b. Fallback: VPN Chi nhánh
     c. Cuối cùng: Cloud VPS
  3. Hỏi mạng lưới: "Có ai giữ tin nhắn của tôi không?" (Gossip query by DID)
  4. Skip-Key Recovery:
     - Nếu Ratchet gap nhỏ: Dynamic Skip Keys khôi phục tin nhắn
     - Nếu Ratchet gap lớn (>7 ngày): Epoch Ratchet ZKP approach
  5. Automated Session Healing:
     - Rust phát hiện MAC mismatch → Signal Control Plane
     - Người gửi re-encrypt bằng Pre-Key mới của máy nhận
  6. UI: Thanh trạng thái "Đang xác thực Zero-Trust..." không block đọc file cũ

Xử lý Out-of-Order Messages:
  - Dynamic Skip Keys: Mobile 2,000 / Laptop 10,000
  - Garbage Collection: Keys bỏ qua >48h → Drop tự động
  - Shimmer animation khi tin nhắn muộn được giải mã và re-ordered
```

### FUNC-04: WebRTC Fallback & ICE Pool Management

```
Luồng khởi tạo Video Call:
  1. .tapp gọi terachat.rtc.request_call() (KHÔNG phép tự tạo UDP socket)
  2. Host Rust ICE Gathering từ Pre-warmed ICE Pool (cập nhật 5 phút/lần hoặc khi đổi mạng)
  3. SDP gửi mDNS candidate (terachat-node-xxxx.local) thay vì IP thực
  4. Timeout 200ms không có ICE response → Restricted NAT detected
  5. Fallback tự động → Internal TURN Server (DMZ của doanh nghiệp)
  6. DTLS-SRTP mã hóa E2EE. TURN Server Zero-Knowledge (chỉ forward packets)
  7. .tapp chỉ nhận Video Frame bytes từ Sandbox boundary

Network Shield Status:
  - "P2P (Direct)" → xanh lá
  - "Relayed (E2EE)" → trắng bạc (tinh thể)
  - "Mesh (BLE only)" → xám (WebRTC disabled)
